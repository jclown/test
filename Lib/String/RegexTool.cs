...
namespace Lib
{
    public static class RegexTool
    {
        ...
        {
            var regex = new Regex(@"((25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))");
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static bool IsCnString(string value)
        {
            Regex cn = new Regex("[\u4e00-\u9fa5]+");
            return cn.IsMatch(value);
        }
        ...
        public static bool CheckNumberListByString(string source)
        {
        ...
        public const string Regex_CheckEmailByString = "^\\s*([A-Za-z0-9_-]+(\\.\\w+)*@([\\w+\\-]+\\.)+\\w{2,5})\\s*$";
        ...
        /// 匹配URL是否包含域名
        /// </summary>
        /// <param name="source">源域名</param>
        /// <param name="url">待检查域名 eg：7224.com</param>
        /// <returns>结果true 匹配 反之不匹配</returns>
        public static bool CheckURLContainsUrl(string source, string url)
        {
            if (string.IsNullOrWhiteSpace(source) || string.IsNullOrWhiteSpace(url)) return false;
            return new Regex("(http|https)://(?<domain>[^(:|/]*)", RegexOptions.IgnoreCase)
                .Match(source).Groups["domain"].Value.ToLower()
                .EndsWith(url);
        }
        ...
        public const string Regex_CheckPasswordByString = @"^(?=.*\d)(?=.*[a-zA-Z])(?=.*[^a-zA-Z0-9]).{8,16}$";
        ...
        {
            Regex rg = new Regex(Regex_CheckPasswordByString);
            return rg.IsMatch(source);
        }
        ...
        public const string Regex_CheckTelephoneByString = @"^((\+\d{1,3}(-| )?\(?\d\)?(-| )?\d{1,5})|(\(?\d{2,6}\)?))(-| )?(\d{3,4})(-| )?(\d{4})(( x| ext)\d{1,5}){0,1}$";
        ...
        {
            Regex rg = new Regex(Regex_CheckTelephoneByString);
            return rg.IsMatch(source);
        }
        ...
        {
            Regex rg = new Regex(@"^[1]+[3,4,5,7,8]+\d{9}$");
            return rg.IsMatch(source);
        }
