...
namespace Lib
{
    ...
    public static class RandomHelper
    {
        ...
        /// <param name="type"> 随机码类型 1：全数字 2：全字符 3：字母+数字</param>
        /// <returns></returns>
        public static string GetRandomString(int length, int type)
        {
            const string bcode = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM";
            const string ncode = "12356890";
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                {
                    case 1:
                        t = new Random().Next(ncode.Length);
                        code.Append(ncode[t]);
                        break;
                    case 2:
                        t = new Random().Next(bcode.Length);
                        code.Append(bcode[t]);
                        break;
                    case 3:
                        t = new Random().Next(bcode.Length + ncode.Length);
                        code.Append((bcode + ncode)[t]);
                        break;
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static string CreateAutoNo(string prefix)
        {
            var no = prefix + DateTime.Now.ToString("yyMMdd") + Lib.RandomHelper.GetRandomString(6, 1);
            return no;
        }
