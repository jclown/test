...
namespace Lib.Mapper
{
    ...
    public static class Copier
    {
        ...
        public static dynamic CopyByBinaryFormatter(dynamic source)
        {
            using (Stream objStream = new MemoryStream())
            {
                // 利用 System.Runtime.Serialization序列化与反序列化完成引用对象的复制
                IFormatter formatter = new BinaryFormatter();
                formatter.Serialize(objStream, source);
                objStream.Seek(0, SeekOrigin.Begin);
                return (dynamic)formatter.Deserialize(objStream);
            }
        }

        public static dynamic CopyByJsonFormatter(dynamic source)
        {
            string valueString = JsonConvert.SerializeObject(source);
            return JsonConvert.DeserializeObject<dynamic>(valueString);            
        }
    }
}
